...
read -p "[3] Pool Host&Port (pool.bytecoin.party:3333) > " pool
read -p "[4] Your BCN wallet (important!!!) > " addr
if [ ! -n "$lport" ];then
    lport="7777"
fi
if [ ! -n "$domain" ];then
...
if [ ! -n "$pool" ];then
    pool="pool.bytecoin.party:3333"
fi
while  [ ! -n "$addr" ];do
    read -p "Plesae set BCN wallet address!!! > " addr
done
read -p "[5] The Pool passwd (x) > " pass
